...
class PollApplicationTests {
	...
	void testScenario() {
		...
		Vote vote1 = new Vote(user2.getId(), voteOption2);

		ResponseEntity<Void> responseVote = client.post()
		...
		assertEquals(HttpStatus.OK, responseVote.getStatusCode());
		...
		assertTrue(response7.getBody().contains(vote1));
